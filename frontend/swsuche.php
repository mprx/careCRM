...
if (isset($_POST["submit"]))
{
    ...
    else
    {
...

        {
            ...
            {
                if (isset($_POST["suche_vn"])) $query_where_list[] = "(sachwalter.sw_vorname LIKE '%$wort%' AND adressen.sw_id = sachwalter.sw_id AND adressen.ad_aktiv = 1 AND adressen.ad_typ IN('rb','r')) AND sw_typ = 'sw'";
                if (isset($_POST["suche_nn"])) $query_where_list[] = "(sachwalter.sw_nachname LIKE '%$wort%' AND adressen.sw_id = sachwalter.sw_id AND adressen.ad_aktiv = 1 AND adressen.ad_typ IN('rb','r')) AND sw_typ = 'sw'";
                if (isset($_POST["suche_ad"])) $query_where_list[] = "(adressen.ad_strasse LIKE '%$wort%' AND adressen.sw_id = sachwalter.sw_id) AND sw_typ = 'sw'";
            }
        }
        ...
        if (mysqli_num_rows($data) != 0)
        {
            ...
            {
                echo "<a href='swdetails.php?sw_id=" . $row["sw_id"] . "'> &bull; " . $row["sw_vorname"] . " " . $row["sw_nachname"] .", " . $row["ad_strasse"] .", " . $row["ad_plz"] . "</a>";
                echo "<br />";
            }
