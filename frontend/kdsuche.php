...
if (isset($_POST["submit"]))
{
    ...
    else
    {
        ...
        $query_start = "SELECT kunde.kd_id, kunde.kd_vorname, kunde.kd_nachname, adressen.ad_strasse, adressen.ad_plz FROM kunde, adressen";
        $query_where_list = array();

        {
            foreach ($suchbegriffe as $wort)
            {
                if (isset($_POST["suche_vn"])) $query_where_list[] = "(kunde.kd_vorname LIKE '%$wort%' AND adressen.kd_id = kunde.kd_id AND adressen.ad_aktiv = 1 AND adressen.ad_typ IN('rb','r'))";
                if (isset($_POST["suche_nn"])) $query_where_list[] = "(kunde.kd_nachname LIKE '%$wort%' AND adressen.kd_id = kunde.kd_id AND adressen.ad_aktiv = 1 AND adressen.ad_typ IN('rb','r'))";
                if (isset($_POST["suche_ad"])) $query_where_list[] = "(adressen.ad_strasse LIKE '%$wort%' AND adressen.kd_id = kunde.kd_id)";
            }
        }
        ...
        if (mysqli_num_rows($data) != 0)
        {
            ...
            {
                echo "<a href='kddetails.php?kd_id=" . $row["kd_id"] . "'> &bull; " . $row["kd_vorname"] . " " . $row["kd_nachname"] .", " . $row["ad_strasse"] .", " . $row["ad_plz"] . "</a>";
                echo "<br />";
            }

            echo "<br />";
            echo "<a href='kdsuche.php'>Neue Suche</a>";
        }
        else
        {
            echo '<div class="alert alert-info" role="alert">';
            echo "Keine Treffer für \"$suchbegriff\". in Kunden";
            echo '</div>';
            $error = 1;
        }
