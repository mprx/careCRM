...
{
    $rg_alle = str_replace("-", ",",mysqli_real_escape_string($dbc, trim($_POST['rg_alle'])));
    ...
    if(!empty($rg_alle))
    {
        $query = "UPDATE rechnung SET rg_bezahlt = 0 WHERE rg_id IN($rg_alle)";
    ...
    $selectquery = "SELECT DISTINCT rg_jahr FROM rechnung ORDER BY rg_jahr";
    ...
    {
        //Daten holen
        $query = "SELECT *, DATE_FORMAT(rg_datum,'%d.%m.%Y') AS rg_datum1 FROM rechnung WHERE rg_jahr = '$rg_jahr' ORDER BY rg_nr DESC, rg_art DESC";
    ...
    <form method="GET" action="<?php echo $_SERVER["PHP_SELF"]; ?>" id="rg_jahr_select">
        <div class="form-group row">
                ...
                <label for="rg_jahr" class="col-sm-1 col-form-label col-form-label-sm">Jahr:</label>
    ...
    <table class="table table-striped table-sm">
        ...
        while($row = mysqli_fetch_array($data))
        {
            ...
            $slash = "-";
            if($i == 1) $slash = "";
            $rg_alle = $rg_alle . $slash . $row["rg_id"];
            echo "<tr";
            if($row["rg_bezahlt"] != 1) echo " class='table-danger' ";
            echo ">";
            ...
            echo "<td>";
            ?>
            <input type="checkbox" <?php if($row["rg_bezahlt"] == 1) echo " checked " ?> name="<?php echo $i; ?>" id="<?php echo $i; ?>" value="<?php echo $row["rg_id"]; ?>">
            <label for="<?php echo $i; ?>" class="col-form-label-sm">bezahlt</label>
            <?php
            echo "</td>";
    ...
    <input type="text" name="rg_alle" id="rg_alle" value="<?php echo $rg_alle; ?>" hidden />
